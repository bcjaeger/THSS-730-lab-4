...
# this is an R package I developed to make presentation of results
# a little easier. You can splice R objects into text using {}, 
# and the result will be rounded automatically based on the magnitude
# of the values.

library(table.glue)
table_glue("the variance (95% CI) of SBP is {var_sbp} ({lwr}, {upr})")
...
with(tox_result,
     table_glue("The proportion (95% CI) is \\
...
with(
  t_test_sbp,
  table_glue("Mean SBP is {estimate} (95% CI: {conf.int[1]}, {conf.int[2]})")
)
...
# formatted:
table_pvalue(t_test_sbp$p.value)
...
with(
  ...
  table_glue(
    "The estimated proportion of mothers with toxemia during \\
     pregnancy was {estimate} (95% CI: {conf.int[1]}, {conf.int[2]}). \\
     Testing whether the estimate is equal to 0.05 yields a p-value \\
     of {table_pvalue(p.value)}")
)
...
with(
  t_test_diff,
  table_glue("Mean difference is {estimate} \\
             (95% CI: {conf.int[1]}, {conf.int[2]})")
)
...
with(
  binom_test,
  table_glue("the estimated proportion who are women is \\
             {estimate} (95% CI: {conf.int[1]}, {conf.int[2]})")
)
...
# Don't use an approximation if it's easy to do an exact test.
binom_test <- binom.test(x = sum(lowbwt$sex==0), n = nrow(lowbwt), p = 0.50)

binom_test$p.value

# what if we had done an approximation?
p <- mean(lowbwt$sex == 0)
n <- nrow(lowbwt)
